...
namespace trkf
{
  ...
  void PropTest::beginJob()
  {
    ...
    for(int isurf = 0; isurf < nsurf; ++isurf) {
      ...
      double x0 = 100.*double(rand()) / double(RAND_MAX);        // (0,100)
      double y0 = 100.*double(rand()) / double(RAND_MAX) - 50.;  // (-50,50)
      double z0 = 1000.*double(rand()) / double(RAND_MAX);       // (0,1000)
      double theta =
	...
	for(int i=0; i<n; ++i) {
	  assert(std::abs(vec1(i) - vec2(i)) <= 1.e-6);
	  for(int j=0; j<n; ++j)
	    assert(std::abs(err1(i,j) - err2(i,j)) <= 1.e-5);
	}
      }
    }
    ...
    vec(4) = 1.;
    err.clear();
    tre0 = trkf::KETrack(psurf2, vec, err, trkf::Surface::FORWARD, 13);
    ...
    tre1 = tre0;
    prop->noise_prop(tre1, psurf3, trkf::Propagator::UNKNOWN, false);
    std::cout << "\nSingle step final track:" << std::endl;
    std::cout << tre1 << std::endl;
