...
namespace opdet {
  ...
  */
  ...
  public:
    ...
    float GetParameter(const ChConfigType_t type, const unsigned int ch);  
    /* { */
    /*   if ( !fInitialized ) */
    /* 	doInitialization(); */
    ...
    const std::map<unsigned int, float>& GetParameter(const ChConfigType_t type);
    //{ return fParams[type]; }
  ...
  protected:
    ...
    std::map< unsigned int, std::map< unsigned int, float > > fParams; // [ Parameter Enum, [ Channel Number, Value ] ]
